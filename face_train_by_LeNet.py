...
from load_face_dataset import load_dataset, resize_image, get_classes,IMAGE_SIZE
...
class Model:
    ...
    #识别人脸
        def face_predict(self, image):    
            ...
            #给出类别预测
                result = np.argmax(self.model.predict(image), axis = 1)       
...
if __name__ == '__main__':
        ...
        dataset = Dataset('F:\\face_identify')
        n, f_dict = get_classes('F:\\face_identify')
        print('The number of classes is %d'%n)
        dataset.load(nb_classes = n)
        #nb_classes = get_classes('F:\\face_identify')

        #训练模型
        model = Model()
        model.build_model(dataset, nb_classes = n)
        model.train(dataset)
